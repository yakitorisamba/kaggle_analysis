...
def initialize_tokenizer():
    ...
    Returns:
        tuple: (tokenizer_obj, tokenizer_mode)
    """
    # SudachiPyの初期化
    tokenizer_obj = dictionary.Dictionary().create()
    # モードCを使用（単語の分割粒度が最も細かい）
...
def extract_and_count_words(text, tokenizer_obj, mode):
    """
    テキストから単語を抽出してカウントする補助関数
    
    Args:
        text (str): 分析対象のテキスト
        tokenizer_obj: Sudachiのtokenizerオブジェクト
        mode: トークナイズモード
